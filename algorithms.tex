...
\begin{frame}{Auxiliary isogeny (\arabic{auxRIIcnt}/\auxRIItotal{})}
    ...
    \vspace{10pt}
    Restricting $E_\mathrm{pk}$ to a subset used in SQIsign
    then $\deg\varphi_\mathrm{sec} \approx p^{1/4}$.

    \vspace{5pt}
    We use this restriction, but
    $$ d > p^{3/4}$$
    is too large for $\varphi_\mathrm{aux}$ (we want $\deg\varphi_\mathrm{aux} < 2^a$).
...
\begin{frame}{Auxiliary isogeny (\arabic{auxRIIcnt}/\auxRIItotal{})}
    ...
    \vspace{10pt}
    \red{Issue} (pointed out by \cite{CCILV2024sqisign2d_attack}): $q$ depends on $N$.
\end{frame}
...
\begin{frame}{Key recovery attack}
